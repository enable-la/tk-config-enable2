...
def getPlates():
    try:
        ...
        script_path, job_structure = reads_and_writes.standardCommand('Get Elements')
        ...
        if not plates_footage:
            nuke.message("No footage found in elements folder.")
            return
        ...
        panel = nuke.Panel("Select an element")
        panel.addEnumerationPulldown("Element", " ".join(file_options))  # Space-separated for dropdown
        ...
        selected_file = panel.value("Element")
